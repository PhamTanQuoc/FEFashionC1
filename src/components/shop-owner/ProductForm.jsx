...
const ProductForm = ({ initialData, isEdit = false }) => {
    ...
    const handleImageUpload = (e) => {
        ...
        // Reset input để có thể chọn lại cùng 1 file nếu muốn (sau khi đã xóa)
        e.target.value = "";
    };
    ...
    const removeImage = (index) => {
        ...
        // Kiểm tra xem đây có phải ảnh cũ (đã có trên server) hay không
        const infoFound = existingImagesInfo.find(
            (info) => info.imageUrl === imageToRemove,
        );

        if (infoFound) {
            // ✅ CHỈ ĐÁNH DẤU CHỜ XÓA: Thêm vào danh sách, KHÔNG gọi API xóa database lúc này
            setRemoveImageIds((prev) => {
                if (!prev.includes(infoFound.imageId)) {
                    return [...prev, infoFound.imageId];
                }
                return prev;
            });
            console.log("📍 Marked existing image for deletion (Database remains untouched until Save):", infoFound.imageId);
        } else {
            // ✅ ẢNH MỚI: Xóa hẳn khỏi danh sách vì chưa có trong database
            setFormData((prev) => ({
                ...prev,
            ...
            setPendingFiles((prev) => {
                const fileToCancel = prev.find((f) => f.previewUrl === imageToRemove);
                if (fileToCancel) {
    ...
    // Hàm hoàn tác việc xóa ảnh cũ (Gỡ khỏi danh sách chờ xóa)
    const undoRemoveImage = (imageId) => {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    {initialData?.rejectReason && (
                        <div className={`p-5 rounded-2xl border flex items-start gap-4 animate-in slide-in-from-top-4 duration-300 ${initialData.approvalStatus === "REJECTED"
                            ? "bg-rose-50 border-rose-100 text-rose-800"
                            : "bg-blue-50 border-blue-100 text-blue-800"
                            }`}>
                            <div className={`p-2 rounded-xl shrink-0 ${initialData.approvalStatus === "REJECTED" ? "bg-rose-500 text-white" : "bg-blue-500 text-white"
                                }`}>
                    ...
                    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            {(formData.images || []).map((img, idx) => {
                                const info = existingImagesInfo.find(
                                    (i) => i.imageUrl === img,
                                );
                                const isPendingDelete = info && removeImageIds.includes(info.imageId);
