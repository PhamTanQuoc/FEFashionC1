...
const Products = () => {
  ...
  const filteredProducts = allProducts.filter((product) => {
    ...
    const matchesCategory =
      categoryFilter === "Tất cả danh mục" || categoryName === categoryFilter;

    const matchesStatus =
      statusFilter === "Tất cả" ||
      (product.isActive ? "Đang hoạt động" : "Tạm ẩn") === statusFilter;

    let matchesStock = true;
    const currentStock = product.stock || 0;

    if (stockFilter === "Hết hàng") matchesStock = currentStock === 0;
    else if (stockFilter === "Sắp hết hàng")
      matchesStock = currentStock > 0 && currentStock < 10;
  ...
  const stats = [
    ...
    },
    {
      ...
      value: allProducts.filter((p) => p.isActive).length,
      trend: "Sẵn sàng",
      icon: FiCheckCircle,
        ...
        {stats.map((stat, idx) => (
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p
                className={`text-xs font-bold leading-relaxed flex items-center gap-1 ${stat.title === "Hết hàng" ? "text-rose-500" : "text-emerald-500"}`}
              >
                {stat.title !== "Tạm ẩn" && <FiTrendingUp className="inline" />}
                {stat.trend}
              </p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              {paginatedProducts.length > 0 ? (
                paginatedProducts.map((product) => (
                  <tr
                    ...
                    className={`hover:bg-slate-50/80 transition-colors group ${selectedProducts.includes(product.productId) ? "bg-blue-50/30" : ""}`}
                  >
                ...
                <tr>
                  <td
                    colSpan="8"
                    className="px-6 py-12 text-center text-slate-400 font-bold uppercase tracking-widest bg-slate-50/30"
                  >
        ...
        <div className="px-6 py-6 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm font-medium text-slate-500">
            ...
            trên <span className="text-slate-800 font-bold">{totalItems}</span>{" "}
            sản phẩm
          </p>
