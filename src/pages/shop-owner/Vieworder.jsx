...
export default function Vieworder() {
  ...
  const handleSearchChange = (e) => {
    ...
    if (isValid) {
      ...
      setErrors((prev) => [
        ...prev,
        { field: "search", message: "Từ khóa quá dài (tối đa 100 ký tự)" },
      ]);
    }
  };
    ...
    try {
      if (isEditing) {
        ...
        showToastNotification(
          `Đã cập nhật đơn hàng ${selectedOrder.id} thành công`,
        );
      } else {
      ...
      showToastNotification(
        "Không thể thực hiện tác vụ. Vui lòng thử lại.",
        "error",
      );
    }
  };
  ...
  const handleView = async (order) => {
    ...
    try {
      const details = await ShopOrderService.getOrderDetail(order.orderId);
      setFullOrderDetail(details);
    } catch (error) {
      console.error("Lỗi khi lấy chi tiết đơn hàng:", error);
      showToastNotification("Không thể tải chi tiết đơn hàng", "error");
    } finally {
  ...
  const handleDeleteSelected = async () => {
    if (
      ...
      try {
        await Promise.all(
          selectedOrders.map((id) => ShopOrderService.deleteOrder(id)),
        );
  ...
  const handleExport = (format) => {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        showToastNotification(
          `Đã tải file ${format.toUpperCase()} thành công!`,
        );
      } catch (err) {
  ...
  const stats = [
    ...
    {
      title: TEXTS.kpiConfirmed,
      value: orders.filter((o) => o.type === "confirmed").length,
      trend: "Chờ xử lý",
      icon: FiTruck,
    ...
    {
      title: "Đơn hủy",
      value: orders.filter((o) => o.type === "cancelled").length,
      trend: "Đã hủy",
      icon: FiCheckCircle,
      color: "text-rose-600",
      bg: "bg-rose-50",
        ...
        {stats.map((stat, idx) => (
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <p
                className={`text-xs font-bold leading-relaxed flex items-center gap-1 ${stat.title === TEXTS.kpiRevenue ? "text-emerald-500" : "text-slate-400"}`}
              >
                {stat.trend}
              </p>
            </div>
          ...
          <div className="flex flex-wrap items-end gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                ...
                <option value="confirmed">{TEXTS.statusConfirmed}</option>
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                ...
                <option value="success">{TEXTS.paymentSuccess}</option>
                <option value="failed">{TEXTS.paymentFailed}</option>
                <option value="refunded">{TEXTS.paymentRefunded}</option>
              </select>
            </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {isLoading && !isLoadingMore ? (
                ...
                <tr>
                  <td
                    colSpan="9"
                    className="px-6 py-20 text-center text-slate-400 font-bold uppercase tracking-widest bg-slate-50/30"
                  >
                    <div className="flex flex-col items-center gap-3">
                ...
                orders.map((o) => (
                  <tr
                    key={o.id}
                    className={`hover:bg-slate-50/80 transition-colors group ${selectedOrders.includes(o.id) ? "bg-blue-50/30" : ""}`}
                  >
                    ...
                    <td className="px-6 py-5">
                      <span className="text-sm font-semibold text-slate-600 block max-w-[150px] truncate">
                        {o.product}
                      </span>
                    </td>
                    <td className="px-6 py-5">
          ...
          <div className="px-6 py-6 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/20">
            <p className="text-sm font-medium text-slate-500">
              ...
              <span className="text-slate-800 font-bold">
                {Math.min((currentPage - 1) * itemsPerPage + 1, totalOrders)}-
                {Math.min(currentPage * itemsPerPage, totalOrders)}
              </span>{" "}
              {TEXTS.of}{" "}
              <span className="text-slate-800 font-bold">{totalOrders}</span>{" "}
              {TEXTS.results}
            </p>
            <div className="flex items-center gap-2">
              <button
      ...
      {showDetailModal && selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="max-h-[70vh] overflow-y-auto custom-scrollbar-mini">
              {orderDetailLoading ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
                        ...
                        <span
                          className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${fullOrderDetail.paymentStatus === "Success" || fullOrderDetail.paymentStatus === "Paid" || fullOrderDetail.paymentStatus === "PAID"
                              ...
                              : fullOrderDetail.paymentStatus === "Unpaid"
                                ? "bg-slate-100 text-slate-500"
                                : fullOrderDetail.paymentStatus === "Failed"
                    ...
                    <div className="md:col-span-2 space-y-1">
                      ...
                      <p className="text-xs font-bold text-slate-600 leading-relaxed">
                        {fullOrderDetail.shippingAddress ||
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="border border-slate-100 rounded-2xl overflow-hidden">
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-50">
                          {fullOrderDetail.items.map((item, idx) => (
                            <tr
                              key={idx}
                  ...
                  <div className="pt-6 border-t border-slate-50 space-y-3">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-bold text-slate-700">
                        {formatCurrency(fullOrderDetail.subTotal)}
                      </span>
                    </div>
                    {fullOrderDetail.discountAmount > 0 && (
                      <div className="flex justify-between items-center text-sm">
                        <span className="font-bold text-rose-400 uppercase tracking-widest">
                          Giảm giá ({fullOrderDetail.discountPercent}%):
                        </span>
                        <span className="font-bold text-rose-500">
                          -{formatCurrency(fullOrderDetail.discountAmount)}
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-bold text-slate-700">30.000đ</span>
                    </div>
                    <div className="flex justify-between items-center bg-blue-50 p-5 rounded-2xl mt-4">
                      <span className="text-sm font-black text-blue-600 uppercase tracking-widest">
                        Tổng thanh toán:
                      </span>
                      <span className="text-2xl font-black text-blue-600">
                        {formatCurrency(fullOrderDetail.totalAmount)}
                      </span>
                    </div>
                    {fullOrderDetail.payment && (
                      <div className="flex items-center gap-2 text-[10px] text-slate-400 font-bold uppercase mt-2 ml-1">
                        <FiDollarSign size={12} />
                        Thanh toán qua:{" "}
                        <span className="text-blue-500">
                          {fullOrderDetail.payment.method}
                        </span>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button
                className="px-8 py-3 rounded-2xl font-bold text-slate-500 border border-slate-200 bg-white hover:bg-slate-50 transition-all active:scale-95 shadow-sm"
                onClick={() => setShowDetailModal(false)}
              >
                {TEXTS.btnClose}
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden p-8 animate-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-500 mt-2 font-medium">
              Bạn có chắc muốn xóa đơn hàng này? Thao tác này không thể khôi
              phục.
            </p>
            <div className="mt-8 flex gap-3">
              <button
                className="flex-1 py-3 font-bold text-slate-500 rounded-2xl hover:bg-slate-50"
                onClick={() => setShowDeleteModal(false)}
              >
                Hủy
              </button>
              <button
                className="flex-1 py-3 bg-rose-500 text-white font-bold rounded-2xl shadow-lg shadow-rose-100 hover:bg-rose-600"
